...
// jsPDF is loaded dynamically in the export handler to reduce initial bundle size
...
export default function AnalyticsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'projects' | 'employees' | 'finance'>('projects');
  ...
  }, [startDate, endDate, compareDate, showComparison, activeTab]);
  ...
  const fetchEmployeeReports = async (employee: EmployeeWorkHoursData) => {
    ...
    try {
      const response = await api.get<PaginatedResponse<EmployeeWorkloadReport>>(
        `/workload-actual?userId=${employee.id}&startDate=${startDate}&endDate=${endDate}&limit=1000`
      );
      setEmployeeReports(response.data.data);
    } catch (err) {
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      {/* Date Range Filters — hidden when Finance tab is active (it has its own) */}
      {activeTab !== 'finance' && (
        <div className="card p-4 mb-6">
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-end">
            <div className="flex-1 min-w-0">
              <label className="block text-sm font-medium text-brown-700 mb-1">{t('analytics.startDate')}</label>
              <input
                type="date"
                value={tempStartDate}
            ...
            <div className="flex-1 min-w-0">
              <label className="block text-sm font-medium text-brown-700 mb-1">{t('analytics.endDate')}</label>
              <input
                type="date"
                value={tempEndDate}
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="h-4 w-4 text-brown-600 rounded border-brown-200"
              />
              <label htmlFor="showComparison" className="text-sm text-brown-700">{t('analytics.compareWith')}</label>
            </div>
            {showComparison && (
              <div className="flex-1 min-w-0">
                <label className="block text-sm font-medium text-brown-700 mb-1">{t('analytics.compareDate')}</label>
                <input
                  type="date"
      ...
      {/* Summary Cards — hidden when Finance tab is active */}
      {activeTab !== 'finance' && (
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="card p-4">
            <div className="text-sm text-brown-400 mb-1">{t('analytics.totalProjects')}</div>
            <div className="text-2xl font-bold text-brown-900">
              {projectsData?.summary.totalProjects || 0}
            </div>
          </div>
          <div className="card p-4">
            <div className="text-sm text-brown-400 mb-1">{t('analytics.activeProjects')}</div>
            <div className="text-2xl font-bold text-green-600">
              {projectsData?.summary.activeProjects || 0}
            </div>
          </div>
          <div className="card p-4">
            <div className="text-sm text-brown-400 mb-1">{t('analytics.totalEmployees')}</div>
      ...
      <div className="flex border-b border-brown-200 mb-6">
        <button
          className={`px-4 py-2 font-medium text-sm border-b-2 transition-colors ${
            activeTab === 'projects'
              ? 'border-brown-700 text-brown-900'
      ...
      {activeTab === 'projects' && (
        <div className="space-y-6">
          ...
          {showComparison && projectsData?.comparison && (
            <div className="card">
              <div className="p-4 border-b border-brown-200">
                <h2 className="text-lg font-semibold text-brown-900">{t('analytics.periodComparison')}</h2>
                <p className="text-sm text-brown-400 mt-1">{t('analytics.comparingWith', { date: compareDate })}</p>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-cream-100">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-medium text-brown-500 uppercase">{t('analytics.project')}</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-brown-500 uppercase">{t('analytics.currentHours')}</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-brown-500 uppercase">{t('analytics.previousHours')}</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-brown-500 uppercase">{t('analytics.change')}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-brown-100">
                    {filteredProjects.map((project, idx) => {
                      ...
                      const changePercent = prev && prev.totalActualHours > 0
                        ...
                        : 0;
                      return (
                        <tr key={project.id} className="hover:bg-cream-100">
                          <td className="px-4 py-3 font-medium text-brown-900">{project.name}</td>
                          <td className="px-4 py-3 text-brown-600">{project.totalActualHours}h</td>
                          <td className="px-4 py-3 text-brown-600">{prev?.totalActualHours || 0}h</td>
          ...
          {/* Projects Table */}
          <div className="card">
            <div className="p-4 border-b border-brown-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <h2 className="text-lg font-semibold text-brown-900">{t('analytics.projectWorkloadDetails')}</h2>

              {/* Status Filter */}
              <div className="flex items-center gap-2">
                <span className="text-sm text-brown-400">{t('analytics.filter')}:</span>
                <div className="flex rounded-[0.4rem] overflow-hidden border border-brown-200">
                  <button
                    onClick={() => setProjectStatusFilter('all')}
                    className={`px-3 py-1.5 text-sm font-medium transition-colors ${
                      projectStatusFilter === 'all'
                        ? 'bg-brown-900 text-cream-500'
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 text-sm font-medium transition-colors border-l border-brown-200 ${
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-brown-100">
                  {filteredProjects.map((project) => (
                    <tr key={project.id} className="hover:bg-cream-100">
                      <td className="px-4 py-3">
                        <div className="font-medium text-brown-900">{project.name}</div>
                      </td>
                      <td className="px-4 py-3 text-brown-600">{project.customerName}</td>
                      <td className="px-4 py-3 text-brown-600">{project.managerName}</td>
                      <td className="px-4 py-3">
                        <span
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 text-brown-600">
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-brown-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {activeTab === 'employees' && (
        <div className="card">
          <div className="p-4 border-b border-brown-200">
            <h2 className="text-lg font-semibold text-brown-900">{t('analytics.employeeHours')}</h2>
            {employeesData && (
          ...
          {/* Employee Summary */}
          {employeesData && (
            <div className="grid grid-cols-3 gap-4 p-4 bg-cream-100 border-b border-brown-200">
              <div className="text-center">
                <div className="text-sm text-brown-400">{t('analytics.avgHoursWorked')}</div>
                <div className="text-lg font-semibold text-brown-900">
                  {employeesData.summary.averageHoursWorked}h
                </div>
              </div>
              <div className="text-center">
                <div className="text-sm text-brown-400">{t('analytics.underworking')}</div>
                <div className="text-lg font-semibold text-red-600">
                  {employeesData.summary.employeesUnderworking}
                </div>
              </div>
              <div className="text-center">
                <div className="text-sm text-brown-400">{t('analytics.overworking')}</div>
          ...
          {employeesData && employeesData.employees.length > 0 ? (
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-brown-100">
                  {employeesData.employees.map((employee) => (
                    ...
                    >
                      <td className="px-4 py-3">
                        <div className="font-medium text-brown-900 flex items-center gap-2">
                      ...
                      <td className="px-4 py-3 text-brown-600">{employee.email}</td>
                      <td className="px-4 py-3 text-brown-600">{employee.totalHoursWorked}h</td>
                      <td className="px-4 py-3 text-brown-600">{employee.expectedHours}h</td>
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
            ...
            <div className="p-6 text-center text-brown-400">{t('analytics.noEmployeeData')}</div>
          )}
        </div>
      )}
      ...
      {showEmployeeReportsModal && selectedEmployee && (
        <div className="fixed inset-0 bg-brown-950/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-cream-50 rounded-[0.4rem] shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-hidden flex flex-col">
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-brown-200">
              ...
              <button onClick={closeEmployeeReportsModal} className="text-brown-400 hover:text-brown-600">
            ...
            <div className="p-4 overflow-y-auto flex-1">
              ...
              <div className="bg-brown-50 border border-brown-200 rounded-[0.4rem] p-4 mb-4">
                <div className="flex justify-between items-center">
                  <div>
                    <div className="text-sm text-brown-600">{t('analytics.totalHours')}</div>
                    <div className="text-2xl font-bold text-brown-700">
                      {selectedEmployee.totalHoursWorked} {t('workload.hours')}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm text-brown-600">{t('analytics.reportsCount')}</div>
              ...
              {loadingReports ? (
                ...
                <div className="text-center py-12">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 mx-auto text-brown-300 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                  <p className="text-brown-400">{t('analytics.noReportsForPeriod')}</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {employeeReports.map((report) => (
                    <div key={report.id} className="border border-brown-200 rounded-[0.4rem] p-4">
                      ...
                      {report.userText && (
                        <p className="text-brown-400 text-sm mb-2 italic">
                          {report.userText}
                        </p>
                      )}
                      ...
                      {report.distributions && report.distributions.length > 0 && (
                        <div className="space-y-1 mt-2">
                          {report.distributions.map((dist) => (
                            <div key={dist.id} className="text-brown-700">
                              <span className="font-medium">{dist.project.name}</span>
                              {dist.description && (
                                <span className="text-brown-300 text-sm"> - {dist.description}</span>
                              )}
                            </div>
                          ))}
                        </div>
            ...
            <div className="flex justify-end gap-2 p-4 border-t border-brown-200">
              <button onClick={closeEmployeeReportsModal} className="px-4 py-2 text-brown-700 hover:bg-cream-100 rounded-[0.4rem]">
                {t('common.close')}
              </button>
            </div>
          </div>
