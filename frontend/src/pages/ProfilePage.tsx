      ...
      <div className="card p-6 mb-6">
        {user ? (
          <div className="space-y-4">
            <div>
              <label className="text-sm text-brown-500">{t('profile.name')}</label>
              <p className="text-lg font-medium">
                {user.firstName} {user.lastName}
              </p>
            </div>
            <div>
              <label className="text-sm text-brown-500">{t('profile.email')}</label>
              <p className="text-lg">{user.email}</p>
            </div>
            <div>
              <label className="text-sm text-brown-500">{t('profile.phone')}</label>
              <p className="text-lg">{user.phone || t('common.notSet')}</p>
            </div>
            <div>
              <label className="text-sm text-brown-500">{t('profile.role')}</label>
              <p className="text-lg">{user.role}</p>
            </div>
          </div>
        ) : (
          <p className="text-brown-500">{t('common.loading')}</p>
        )}
      </div>
      ...
      <div className="card p-6">
        ...
        {!showChangePassword ? (
          ...
          <form onSubmit={handleChangePassword} className="space-y-4">
            {error && (
              <div className="p-3 bg-red-50 border border-red-200 rounded-[0.4rem] text-red-700 text-sm">
                {error}
              </div>
            )}
