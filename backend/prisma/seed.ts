import { PrismaClient, UserRole, CompanyType, ChatRole, ChatRequestType, ProjectType, ProjectStatus } from '@prisma/client';
...
async function main() {
  ...
  const admin = await prisma.user.upsert({
    where: { email: 'admin@lencondb.ru' },
    update: {},
    create: {
      firstName: 'Admin',
      lastName: 'User',
      email: 'admin@lencondb.ru',
      phone: '+1234567890',
  ...
  const manager = await prisma.user.upsert({
    where: { email: 'manager@lencondb.ru' },
    update: {},
    create: {
      firstName: 'Manager',
      lastName: 'User',
      email: 'manager@lencondb.ru',
      phone: '+1234567891',
  ...
  const employee = await prisma.user.upsert({
    where: { email: 'employee@lencondb.ru' },
    update: {},
    create: {
      firstName: 'Employee',
      lastName: 'User',
      email: 'employee@lencondb.ru',
      phone: '+1234567892',
  ...
  const trial = await prisma.user.upsert({
    where: { email: 'trial@lencondb.ru' },
    update: {},
    create: {
      firstName: 'Trial',
      lastName: 'User',
      email: 'trial@lencondb.ru',
      phone: '+1234567893',
